...
class MultiTaskUncertaintyLossKendall(nn.Module):
    ...
    def forward(self, losses):
        ...
        ), "Expected {} losses to weight, got {}".format(self.num_tasks, len(losses))
        # factor = torch.pow(2*torch.exp(self.eta) - 2, -1)
        factor = torch.exp(-self.eta) / 2.0
...
class MultiTaskUncertaintyLossLiebel(nn.Module):
    ...
    def forward(self, losses):
        ...
        factor = 1.0 / (2 * self.sigma2)
        reg = torch.log(1.0 + self.sigma2)  # regularisation term
...
class DisturbLabelLoss(nn.Module):
    ...
    def forward(self, pred, target):
        with torch.no_grad():
            ...
            target[disturb_indexes] = torch.randint(
                pred.shape[-1], (int(disturb_indexes.sum()),)
            ).to(self.device)
